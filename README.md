...
- Rust 后台同步 supervisor：自动收件账户独立保持 IMAP `IDLE` 监听，同步期间仍接收推送，重连后立即补查；另在每轮完成后按桌面 60 秒、移动端 120 秒兜底检查，覆盖推送漏报和不支持 `IDLE` 的服务器。临时断线或协议错误会自动重试，认证或本地凭据缺失需更新授权码。
- 自动收信优先获取收件箱邮件头，入库后立即刷新列表、计数和新邮件提醒，正文按需加载；各文件夹的新邮件处理后再推进历史回填，已缓存正文继续保留。
...
- 桌面托盘常驻、可选登录启动；Android 前台收信服务支持独立启动引擎，配合系统任务和网络恢复自动检查。详见 [后台收信与平台边界](docs/background-mail.md)。
- 桌面单实例保护；界面和后台服务共用账户同步锁，重复打开不会启动第二套收信引擎。
...
仓库和 CI 不保存真实授权码；协议 smoke test 必须在本机使用用户自己的凭据，并检查重启后离线读取、Sent 副本和重复同步。当前版本不支持 OAuth 或 POP3/JMAP。Android 在系统允许的前台服务运行期间保持收信连接，服务到达系统时限后由定期任务继续检查；具体限制见 [后台收信与平台边界](docs/background-mail.md)。
